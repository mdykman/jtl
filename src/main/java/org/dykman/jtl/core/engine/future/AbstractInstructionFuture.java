...
public abstract class AbstractInstructionFuture implements InstructionFuture<JSON> {
	...
	@Override
	public final ListenableFuture<JSON> call(final AsyncExecutionContext<JSON> context,
		...
		return transform(data, new AsyncFunction<JSON, JSON>() {
			...
			public ListenableFuture<JSON> apply(JSON input) throws Exception {
				if(input.getType() == JSONType.FRAME) {
					...
					for(JSON j:frame) {
						try {
						ListenableFuture<JSON> r = callItem(context, immediateCheckedFuture(j));
						 	rr.add(callItem(context, immediateCheckedFuture(j)));
						} catch(ExecutionException e) {
							rr.add(immediateFailedCheckedFuture(e));
						}
					}
					return transform(allAsList(rr), new AsyncFunction<List<JSON>, JSON>() {

						@Override
						public ListenableFuture<JSON> apply(List<JSON> _input)
								throws Exception {
							Frame newf = frame.builder().frame();
							for(JSON j:_input) {
